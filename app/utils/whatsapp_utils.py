...
from app.utils.messages import (
    get_greetings_message_input, 
    get_text_message_input, 
    get_menu_message_input,
    get_raise_trade_message_input, 
    get_approve_trade_message_input, 
    get_view_trade_history_message_input,
    get_trade_details_message_input
)
...
def handle_list_reply(interactive):
    ...
    if list_reply["id"] == "1":
        ...
        data = get_text_message_input(current_app.config["RECIPIENT_WAID"], list_reply["title"] + " selected")
        send_message(data)
    elif list_reply["id"] == "initiateTrade":
        data = get_raise_trade_message_input(current_app.config["RECIPIENT_WAID"])
        send_message(data)
    elif list_reply["id"] == "viewTradeHistory":
        data = get_view_trade_history_message_input(current_app.config["RECIPIENT_WAID"])
        send_message(data)
    elif list_reply["id"].find("viewTrade_") != -1:
        data = get_trade_details_message_input(current_app.config["RECIPIENT_WAID"], list_reply["id"])
        send_message(data)
    elif list_reply["id"].find("approveTrade") != -1:
        data = get_text_message_input(current_app.config["RECIPIENT_WAID"], "Trade approved")
        trade_id = list_reply["id"].split("_")[1]
        storage.update_trade_status(trade_id, "approved")
        send_message(data)
    elif list_reply["id"].find("rejectTrade") != -1:
        data = get_text_message_input(current_app.config["RECIPIENT_WAID"], "Trade rejected")
        trade_id = list_reply["id"].split("_")[1]
        storage.update_trade_status(trade_id, "rejected")
        send_message(data)
    else:
